...
float raySphereIntersection(const Vec2 &origin, const Vec2 &dir,
...
int main() {
  ...
  SDL_Window *window =
      SDL_CreateWindow("2D Raycasting Sphere", SDL_WINDOWPOS_CENTERED,
  ...
  while (running) {
    ...
    drawFilledCircle(renderer, static_cast<int>(sourcePos.x),
                     static_cast<int>(sourcePos.y),
                     static_cast<int>(sourceRadius));

    // Disegna ostacolo (terra) come cerchio blu
    SDL_SetRenderDrawColor(renderer, 0, 100, 255, 255);
    drawFilledCircle(renderer, static_cast<int>(blockerPos.x),
                     static_cast<int>(blockerPos.y),
                     static_cast<int>(blockerRadius));
    ...
    for (int i = 0; i < numRays; ++i) {
      ...
      float hitDist =
          raySphereIntersection(sourcePos, dir, blockerPos, blockerRadius);
      ...
      SDL_RenderDrawLine(renderer, static_cast<int>(sourcePos.x),
                         static_cast<int>(sourcePos.y),
